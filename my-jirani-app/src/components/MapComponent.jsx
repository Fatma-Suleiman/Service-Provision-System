...
const AdjustMapBounds = ({ services }) => {
  ...
  useEffect(() => {
    if (services.length > 0) {
      const bounds = L.latLngBounds(services.map(service => [service.lat, service.lon]));
        ...
        <MarkerClusterGroup>
          {services.map((service) => {
            const distance = userCoords
              ? calculateDistance(userCoords.lat, userCoords.lon, service.lat, service.lon).toFixed(2)
              : null;
                ...
                <Popup>
                  <div style={{ lineHeight: '1.5' }}>
                    ...
                    {service.phone_number && (
                      <>
                        <strong>Phone:</strong>{' '}
                        <a href={`tel:${service.phone_number}`}>{service.phone_number}</a> <br />
                      </>
                    )}
                    {distance && (
                      <>
